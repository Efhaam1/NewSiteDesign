...
async function boot() {
  const [stages, catalog, showcase, variance, pricing] = await Promise.all([
    json('/NewSiteDesign/data/stages.json'), json('/NewSiteDesign/data/catalog.json'),
    json('/NewSiteDesign/data/showcase.json'), json('/NewSiteDesign/data/variance.json'),
    json('/NewSiteDesign/data/pricing.json'),
  ]);
